...
export default function ProcessQualityDashboard() {
  ...
  useEffect(() => {
    if (!isBellavita || !clientId) { setBellavitaCQ(null); return; }
    api.get<{ data: typeof bellavitaCQ }>(`/quality/bellavita-cq-score?startDate=${sd}&endDate=${ed}${campaignQs}`)
      .then(r => setBellavitaCQ(r.data?.data ?? null))
  ...
  const [gncCQ, setGncCQ] = useState<{
    overallScore: number; totalCalls: number;
    byAgent: { agentId: string; agentName: string; callCount: number; avgScore: number }[];
  } | null>(null);
  useEffect(() => {
    if (!isGnc || !clientId) { setGncCQ(null); return; }
    api.get<{ data: typeof gncCQ }>(`/quality/gnc-cq-score?startDate=${sd}&endDate=${ed}${campaignQs}`)
      .then(r => setGncCQ(r.data?.data ?? null))
      .catch(() => setGncCQ(null));
  ...
  useEffect(() => {
    if (!isBellavita || !clientId || activeSlide !== 8) return;
    setBellavitaCQDetailsLoading(true);
    api.get<{ data: typeof bellavitaCQDetails }>(`/quality/bellavita-cq-score/details?startDate=${sd}&endDate=${ed}${campaignQs}`)
  ...
  return (
    <div className="min-h-screen text-slate-900 flex flex-col">
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-5 w-full space-y-5">
        ...
        {/* ─── Slide 8: CQ Score (Bellavita only) ──────────────────────────── */}
        {activeSlide === 8 && isBellavita && clientId && (() => {
          const PARAM_LABELS: { key: keyof NonNullable<typeof bellavitaCQDetails>['paramPassRate']; label: string }[] = [
            { key: 'opening', label: 'Opening' },
            { key: 'offered', label: 'Offered' },
            { key: 'objectionHandling', label: 'Objection Handling' },
          ...
          return (
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-2xl border border-violet-200 bg-white overflow-hidden shadow-sm">
                ...
                <div className="flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-slate-200">
                  {/* Overall score */}
                  <div className="lg:w-60 shrink-0 p-6 flex flex-col items-center justify-center gap-1 bg-gradient-to-b from-violet-50/60 to-white">
                    <svg width="128" height="128" viewBox="0 0 128 128" className="mb-1">
                      <circle cx="64" cy="64" r="54" fill="none" stroke="#eef2f1" strokeWidth="12" />
                      <circle cx="64" cy="64" r="54" fill="none"
                        stroke={(bellavitaCQ?.overallScore ?? 0) >= 85 ? '#22b990' : (bellavitaCQ?.overallScore ?? 0) >= 60 ? '#eea12b' : '#e8607d'}
                        strokeWidth="12" strokeLinecap="round"
                        strokeDasharray={`${((bellavitaCQ?.overallScore ?? 0) / 100) * 339.3} 339.3`}
                        transform="rotate(-90 64 64)" />
                      <text x="64" y="58" textAnchor="middle" fontSize="26" fontWeight="800" fill="#15212d">{bellavitaCQ?.overallScore ?? 0}%</text>
                      <text x="64" y="78" textAnchor="middle" fontSize="9" fontWeight="700" fill="#71808c">OVERALL</text>
                    </svg>
                    <p className="text-[10px] text-slate-500 font-semibold">
                      {(bellavitaCQDetails?.totalCalls ?? 0).toLocaleString()} call{(bellavitaCQDetails?.totalCalls ?? 0) === 1 ? '' : 's'} in this period
                    </p>
                  </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
                <div className="flex items-center gap-3 px-4 py-2.5 border-b border-slate-200">
                  ...
                  <ExportBtn onClick={() => downloadCSV(
                    ...
                    'bellavita-agent-wise-parameters-score.csv',
                  )} />
                </div>
                <div className="overflow-x-auto max-h-[32rem] overflow-y-auto">
                  <table className="w-full text-[11px] whitespace-nowrap border-separate border-spacing-0">
                    <thead className="sticky top-0 z-10">
                      <tr className="text-slate-500 uppercase text-[9px] tracking-wider">
                        ...
                        <th className="px-3 py-3 text-center bg-violet-50 border-b border-l border-slate-200 text-violet-700">Overall CQ %</th>
                        <th className="px-3 py-3 text-left bg-slate-50 border-b border-l border-slate-200">Weakest Area</th>
                      </tr>
                    </thead>
                    <tbody>
                      {bellavitaCQDetailsLoading ? (
                        <tr><td colSpan={10} className="px-4 py-10 text-center text-slate-400 border-b border-slate-100">
                          <Loader2 size={16} className="inline animate-spin mr-2" /> Loading...
                        </td></tr>
                      ) : filteredCqAgents.length === 0 ? (
        ...
        {/* ─── Slide 10: CQ Score (GNC only) ───────────────────────────────── */}
        {activeSlide === 10 && isGnc && clientId && (() => {
          const PARAM_LABELS: { key: keyof NonNullable<typeof gncCQDetails>['paramPassRate']; label: string }[] = [
            { key: 'opening', label: 'Opening' },
            { key: 'offered', label: 'Offered' },
            { key: 'objectionHandling', label: 'Objection Handling' },
            { key: 'prepaidPitch', label: 'Prepaid Pitch' },
          ...
          return (
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-2xl border border-amber-200 bg-white overflow-hidden shadow-sm">
                <div className="px-5 py-3 flex items-center gap-2" style={{ background: 'linear-gradient(135deg,#D97706,#92400E)' }}>
                  <Target size={14} className="text-white" />
                  <span className="text-[11px] font-bold uppercase tracking-widest text-white">CQ Score Overview</span>
                  <span className="ml-auto text-[9px] text-white/75 font-semibold">
                    (Opening + Offered + ObjectionHandling + PrepaidPitch + UpsellingEfforts + OfferUrgency) ÷ 6
                ...
                <div className="flex flex-col lg:flex-row divide-y lg:divide-y-0 lg:divide-x divide-slate-200">
                  {/* Overall score */}
                  <div className="lg:w-60 shrink-0 p-6 flex flex-col items-center justify-center gap-1 bg-gradient-to-b from-amber-50/60 to-white">
                    <svg width="128" height="128" viewBox="0 0 128 128" className="mb-1">
                      <circle cx="64" cy="64" r="54" fill="none" stroke="#eef2f1" strokeWidth="12" />
                      <circle cx="64" cy="64" r="54" fill="none"
                        stroke={(gncCQ?.overallScore ?? 0) >= 85 ? '#22b990' : (gncCQ?.overallScore ?? 0) >= 60 ? '#eea12b' : '#e8607d'}
                        strokeWidth="12" strokeLinecap="round"
                        strokeDasharray={`${((gncCQ?.overallScore ?? 0) / 100) * 339.3} 339.3`}
                        transform="rotate(-90 64 64)" />
                      <text x="64" y="58" textAnchor="middle" fontSize="26" fontWeight="800" fill="#15212d">{gncCQ?.overallScore ?? 0}%</text>
                      <text x="64" y="78" textAnchor="middle" fontSize="9" fontWeight="700" fill="#71808c">OVERALL</text>
                    </svg>
                    <p className="text-[10px] text-slate-500 font-semibold">
                      {(gncCQDetails?.totalCalls ?? 0).toLocaleString()} call{(gncCQDetails?.totalCalls ?? 0) === 1 ? '' : 's'} in this period
                    </p>
                  </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
                <div className="flex items-center gap-3 px-4 py-2.5 border-b border-slate-200">
                  ...
                  <ExportBtn onClick={() => downloadCSV(
                    ...
                    'gnc-agent-wise-parameters-score.csv',
                  )} />
                </div>
                <div className="overflow-x-auto max-h-[32rem] overflow-y-auto">
                  <table className="w-full text-[11px] whitespace-nowrap border-separate border-spacing-0">
                    <thead className="sticky top-0 z-10">
                      <tr className="text-slate-500 uppercase text-[9px] tracking-wider">
                        ...
                        <th className="px-3 py-3 text-center bg-amber-50 border-b border-l border-slate-200 text-amber-700">Overall CQ %</th>
                        <th className="px-3 py-3 text-left bg-slate-50 border-b border-l border-slate-200">Weakest Area</th>
                      </tr>
                    </thead>
                    <tbody>
                      {gncCQDetailsLoading ? (
                        ...
                        return (
                          <tr key={a.agentId} className={i % 2 === 1 ? 'bg-slate-50/50' : undefined}>
                            ...
                            <td className="px-3 py-2.5 text-center border-b border-l border-slate-100 bg-amber-50/30">
                              <span className="inline-block min-w-[52px] px-2 py-1 rounded-md text-xs font-black tabular-nums"
                                style={{ color: overallC.text, backgroundColor: overallC.bg }}>
                                {a.overallScore}%
                              </span>
