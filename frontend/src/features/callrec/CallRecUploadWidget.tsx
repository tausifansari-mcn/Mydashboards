...
// Same shape as frontend/src/features/sales/UploadLog.tsx (drag-drop, upload, history, revert) —
// kept as its own copy rather than a shared import so this feature doesn't reach into the sales
// feature folder for an accent-color context it has no reason to depend on.
export function CallRecUploadWidget({ endpoint, table, title }: Props) {
  ...
  const fetchLogs = useCallback(async () => {
    setLoadingLogs(true);
    try {
      const { data } = await api.get('/call-rec-upload/upload-logs', { params: { table } });
      setLogs(data.data ?? []);
    } catch { setLogs([]); }
  ...
  const upload = async () => {
    ...
    try {
      ...
      const { rowsInserted, totalRows } = data.data;
      setResult({
        success: true,
        message: rowsInserted < totalRows
          ? `Uploaded ${rowsInserted.toLocaleString()} of ${totalRows.toLocaleString()} rows (${totalRows - rowsInserted} skipped as blank/invalid)`
          : `Uploaded ${rowsInserted.toLocaleString()} rows`,
      });
      setFile(null);
  ...
  const deleteLog = async (batchId: string) => {
    if (!confirm('Delete this upload and revert its rows?')) return;
    try {
      await api.delete(`/call-rec-upload/upload-log/${batchId}`, { params: { table } });
      fetchLogs();
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      >
        <div className="pointer-events-none absolute -right-16 -top-16 h-48 w-48 rounded-full opacity-[0.06] blur-2xl" style={{ background: ACCENT }} />
        <div className="relative flex items-center gap-3 mb-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl shrink-0" style={{ background: `linear-gradient(135deg, ${ACCENT}, ${ACCENT}dd)` }}>
        ...
        <div
          ...
          style={{
            borderColor: dragOver ? ACCENT : '#e2e8f0',
            backgroundColor: dragOver ? `${ACCENT}0d` : '#f8fafc',
        ...
        <AnimatePresence>
          {file && (
            <motion.button
              ...
              style={{ background: `linear-gradient(135deg, ${ACCENT}, ${ACCENT}dd)`, boxShadow: `0 8px 20px -8px ${ACCENT}80` }}
            >
              ...
              {logs.map((log) => (
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg shrink-0" style={{ backgroundColor: `${ACCENT}15` }}>
                      <FileSpreadsheet className="h-3.5 w-3.5" style={{ color: ACCENT }} />
                    </div>
                    <div className="min-w-0">
                      <p className="font-medium text-slate-700 truncate">{log.fileName}</p>
                      <p className="text-slate-400">{log.rowsInserted.toLocaleString()} rows &middot; {new Date(log.uploadedAt).toLocaleString()}</p>
                    </div>
                  </div>
