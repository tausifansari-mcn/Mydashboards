...
export default function GncAprUpload() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      fd.append('file', file);
      const res = await api.post('/sales/upload-gnc-apr', fd, {
        onUploadProgress: (e) => {
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-sm font-semibold text-slate-800">GNC APR Upload</h2>
      </div>

      <div
        onDrop={handleDrop}
        onDragOver={e => e.preventDefault()}
        onClick={() => document.getElementById('gnc-apr-input')?.click()}
