...
import { BRAND_THEMES, BrandTheme } from './brandThemes';
...
function SectionCard({
  title, accent = '#3B82F6', headerBg, headerText, children, onDownload,
}: {
  title: string; accent?: string; headerBg?: string; headerText?: string;
    ...
    <>
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
        <div className="flex items-center gap-2.5 px-5 py-3 border-b border-slate-200"
          style={{ background: headerBg || '#F8FAFC' }}>
          <div className="w-1.5 h-4 rounded-full shrink-0" style={{ backgroundColor: accent }} />
          <h3 className="text-xs font-semibold uppercase tracking-widest flex-1"
            style={{ color: headerText || '#64748B' }}>{title}</h3>
          <div className="flex items-center gap-1">
            {onDownload && (
              <button onClick={onDownload} title="Download chart data as CSV"
                className="p-1.5 rounded-lg transition-colors"
                style={{ color: headerText ? `${headerText}99` : '#94A3B8', colorAdjust: 'exact' }}
            ...
            <button onClick={() => setExpanded(true)} title="Expand"
              className="p-1.5 rounded-lg transition-colors"
              style={{ color: headerText ? `${headerText}99` : '#94A3B8' }}>
              <Maximize2 size={13} />
            </button>
          </div>
        </div>
          ...
          <div className="flex flex-col bg-white rounded-2xl border border-slate-200 overflow-hidden flex-1 min-h-0">
            <div className="flex items-center gap-2.5 px-6 py-4 border-b border-slate-200 shrink-0"
              ...
              <h3 className="text-sm font-semibold flex-1"
                style={{ color: headerText || '#475569' }}>{title}</h3>
              {onDownload && (
...
const PROJECT_META: Record<string, { name: string; icon: string; color: string; mandate: number; required: number; hasFCR: boolean }> = {
  gnc:          { name: 'GNC',          icon: '🛒', color: '#CE112D', mandate: 8,  required: 6,  hasFCR: false },
  bellavita:    { name: 'Bellavita',    icon: '🌸', color: '#D4AF37', mandate: 14, required: 12, hasFCR: false },
  clovia:       { name: 'Clovia',       icon: '👗', color: '#E40B92', mandate: 7,  required: 6,  hasFCR: false },
  neemans:      { name: 'Neemans',      icon: '👟', color: '#6B8E4E', mandate: 10, required: 10, hasFCR: true  },
  viega:        { name: 'Viega',        icon: '🚰', color: '#FFD100', mandate: 2,  required: 2,  hasFCR: false },
  exicom:       { name: 'Exicom',       icon: '⚡', color: '#003366', mandate: 5,  required: 5,  hasFCR: false },
  dubangladesh: { name: 'DU Bangladesh',icon: '🇧🇩', color: '#003D6B', mandate: 3,  required: 3,  hasFCR: false },
};
...
export default function InboundProjectDashboard() {
  ...
  const theme: BrandTheme | undefined = BRAND_THEMES[projectKey];
  ...
  return (
    <div className="min-h-screen p-6 space-y-6" style={{ background: t.bg }}>

      {/* Header */}
      <div className="flex items-center justify-between flex-wrap gap-3 rounded-2xl p-5"
        style={{ background: t.headerBg }}>
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/inbound')}
            className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition-all hover:opacity-80"
            style={{
              background: t.style === 'dark' ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.08)',
              color: t.headerText,
            }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold" style={{ color: t.headerText }}>{meta.name} Inbound</h1>
              <p className="text-xs" style={{ color: `${t.headerText}AA` }}>Mandate: {meta.mandate} agents · Required: {meta.required}</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {(['today','7d','30d'] as const).map(r => (
            <button key={r} onClick={() => setRange(r)}
              ...
              style={{
                background: range === r ? t.activeBtn : 'rgba(255,255,255,0.1)',
                color: range === r ? '#FFFFFF' : `${t.headerText}AA`,
                border: range === r ? 'none' : `1px solid ${t.headerText}33`,
              }}>
              {r==='today'?'Today':r==='7d'?'Last 7 Days':'Last 30 Days'}
            </button>
          ))}
          <button onClick={fetchAll}
            className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs transition-colors hover:opacity-80"
            style={{
              background: 'rgba(255,255,255,0.1)',
              color: t.headerText,
              border: `1px solid ${t.headerText}33`,
            }}>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        <KPICard label="Offered"  value={(summary?.offered??0).toLocaleString()}  sub="Total calls"       color={t.primary}                       icon={Phone}         />
        <KPICard label="Answered" value={(summary?.answered??0).toLocaleString()} sub="Answered by agents" color={t.chartColor2}                    icon={PhoneIncoming} />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          <SectionCard title="Hourly Call Distribution · Today"
            accent={t.accent} headerBg={t.sectionHeaderBg} headerText={t.sectionHeaderText}
            ...
            {hourly.length === 0 ? (
              ...
              <ChartContainer>
                <BarChart data={hourly} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="offered"  name="Offered"  fill={t.chartColor} radius={[3,3,0,0]} />
                  <Bar dataKey="answered" name="Answered" fill={t.chartColor2} radius={[3,3,0,0]} />
                </BarChart>
              </ChartContainer>
            )}
          </SectionCard>
      ...
      {/* AL% & SL% trend */}
      <SectionCard title="Daily AL% & SL% Trend · Last 30 Days"
        accent={t.accent} headerBg={t.sectionHeaderBg} headerText={t.sectionHeaderText}
        onDownload={() => exportCSV(
        ...
        {trend.length === 0 ? (
          ...
          <ChartContainer>
            <LineChart data={trend} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey="al" name="AL%" stroke={t.chartColor} strokeWidth={2} dot={false} />
              <Line type="monotone" dataKey="sl" name="SL%" stroke={t.chartColor2} strokeWidth={2} dot={false} />
      ...
      {/* ACHT & Repeat trend */}
      <SectionCard title="Daily ACHT & Repeat% Trend · Last 30 Days"
        accent={t.accent} headerBg={t.sectionHeaderBg} headerText={t.sectionHeaderText}
        onDownload={() => exportCSV(
          ['Date','ACHT(s)','Repeat%'],
        ...
        {trend.length === 0 ? (
          ...
          <ChartContainer>
            <ComposedChart data={trend} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
              ...
              <Line yAxisId="acht"   type="monotone" dataKey="acht"       name="ACHT"    stroke={t.chartColor} strokeWidth={2} dot={false} />
              <Line yAxisId="repeat" type="monotone" dataKey="repeat_pct" name="Repeat%" stroke={t.chartColor2} strokeWidth={2} dot={false} />
            </ComposedChart>
          </ChartContainer>
        )}
      </SectionCard>
      ...
      {/* Daily Call Volume */}
      <SectionCard title="Daily Call Volume · Last 30 Days"
        accent={t.accent} headerBg={t.sectionHeaderBg} headerText={t.sectionHeaderText}
        onDownload={() => exportCSV(
          ...
          <ChartContainer>
            <BarChart data={trend} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
              ...
              <Tooltip contentStyle={{ backgroundColor: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 8, fontSize: 11, boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}
                formatter={(v: unknown, n: unknown) => [Number(v).toLocaleString(), String(n)]} />
              <Legend wrapperStyle={{ fontSize: 12 }} />
              <Bar dataKey="offered"  name="Offered"  fill={t.chartColor} radius={[2,2,0,0]} />
              <Bar dataKey="answered" name="Answered" fill={t.chartColor2} radius={[2,2,0,0]} />
            </BarChart>
          </ChartContainer>
        )}
      </SectionCard>
      ...
      <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
        <div className="flex items-center gap-2.5 px-5 py-3 border-b border-slate-200"
          style={{ background: t.sectionHeaderBg }}>
          <div className="w-1.5 h-4 rounded-full shrink-0" style={{ backgroundColor: t.accent }} />
          <h3 className="text-xs font-semibold uppercase tracking-widest flex-1"
            style={{ color: t.sectionHeaderText }}>
          ...
          <button onClick={exportPerf}
            ...
            style={{ color: `${t.sectionHeaderText}CC` }}>
